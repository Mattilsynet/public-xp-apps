...
const getImmutableUrl = ({ manifestPath = FILEPATH_MANIFEST_ESM, path, urlPrefix }: UrlParams) => {
  if (IS_DEV_MODE) {
...
export const getSiteUrl = ({ manifestPath = FILEPATH_MANIFEST_ESM, path }: UrlPostfixParams) => {
...
export const getBaseWebappUrl = (path?: string) => {
  ...
  const base = vhostsEnabled() && webappVhost ? webappVhost.source : `/webapp/${app.name}`

  return path ? `${base}/${path}` : base
}

export const getWebappUrl = ({ manifestPath = FILEPATH_MANIFEST_ESM, path }: UrlPostfixParams) => {
  if (IS_DEV_MODE) {
    manifests[manifestPath] = ioResource(manifestPath)
  }
