...
import { DIR_DST_STATIC, DIR_SRC_STATIC } from './constants'

export default function buildStaticConfig(): Options {
	const GLOB_EXTENSIONS_STATIC = '{ts,jsx,js}'
	...
	return {
		...
		esbuildPlugins: [
			CopyWithHashPlugin({
				...
				patterns: ['react/{cjs,umd}/*.js', 'react-dom/{cjs,umd}/*.js'],
			}),
			GlobalsPlugin({
				react: 'React',
			}),
